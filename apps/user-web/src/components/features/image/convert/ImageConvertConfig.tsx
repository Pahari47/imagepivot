...
export function ImageConvertConfig({ config, onChange }: FeatureConfigProps) {
  ...
  // For "Convert To PNG", format is always PNG
  // For "Convert From PNG", show format selection (excluding PNG)
  const availableFormats = conversionType === 'to' 
    ? FORMATS.filter(f => f.value === 'png')
    : FORMATS.filter(f => f.value !== 'png');
